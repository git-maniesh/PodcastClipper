...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      return new NextResponse("Webhook sinature verification fialed", {
        status: 400,
      });
    }
    if (event.type === "checkout.session.completed") {
      ...
      const retreiveSession = await stripe.checkout.sessions.retrieve(
        session.id,
        { expand: ["line_items"] },
      );
      const lineItems = retreiveSession.line_items;
