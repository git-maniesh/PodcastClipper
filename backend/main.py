...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    if not tracks_path or not scores_path:
        print("DEBUG: tracks =", tracks_path)
        print("DEBUG: scores =", scores_path)
        raise FileNotFoundError("Tracks or scores not found for clips")
    ...
    s3_client = boto3.client(
        "s3",
        # check your Backblaze bucket region
        endpoint_url="https://s3.us-east-005.backblazeb2.com",
        aws_access_key_id=os.environ["BACKBLAZE_ACCESS_KEY_ID"],
        aws_secret_access_key=os.environ["BACKBLAZE_SECRET_ACCESS_KEY"],
        region_name="us-east-005"
    )
    # s3_client.upload_file(vertical_mp4_path,'ai-podcast-clipper',output_s3_key)
    s3_client.upload_file(str(subtitle_output_path),
                          'ai-podcast-clipper', output_s3_key)
...
class AiPodcastClipper:
    ...
    def load_model(self):
        ...
        print("Transcripition models loaded...")
    ...
    # def transcribe_video(self, base_dir: str, video_path: str) -> str:
        ...
        # print("STarting traanscription with whispersxx")
    ...
    def transcribe_video(self, base_dir: str, video_path: str, fallback_lang: str = None) -> str:
        audio_path = base_dir / "audio.wav"
        extract_cmd = f"ffmpeg -i {video_path} -vn -acodec pcm_s16le -ar 16000 -ac 1 {audio_path}"
    ...
    # Step 1: transcribe (auto-detects language if not specified)
        if fallback_lang:
            result = self.whisperx_model.transcribe(
                audio, batch_size=16, language=fallback_lang)
            print(f"Forcing language: {fallback_lang}")
        else:
            result = self.whisperx_model.transcribe(audio, batch_size=16)
            print(f"Auto-detected language: {result['language']}")

    # Step 2: alignment model
        detected_lang = result["language"] if not fallback_lang else fallback_lang
        ...
        try:
            ...
            print(
                f"⚠️ Alignment model not available for {detected_lang}, falling back to English")
            self.alignment_model, self.metadata = whisperx.load_align_model(
                language_code="en",
                device="cuda"
            )
    ...
    # Step 3: align
        ...
        # print(json.dumps(result_aligned, indent=2,
        #                  ensure_ascii=False))  # show real words
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        # 2. Identify the moments for clips
...
def main():
    ...
    payload = {
        "s3_key": "test1/bill5.mp4"
    }

    headers = {
        "Content-Type": "application/json",
        "Authorization": "Bearer 1234353"
    }
...
# keyID:
# 0058da6986a06010000000002
# keyName:
# ai-podcast-clipper-user
# applicationKey:
# K0055wgEmYg+/NEQip0o+adxHQfhGJk
